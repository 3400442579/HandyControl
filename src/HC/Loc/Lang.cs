...
namespace Loc
{
    public class Lang : LangFields
    {
        ...
        Lang()
        {
            ...
            _defaultLanguage = LoadLang("zh-CN");
        ...
        Dictionary<string, string> LoadLang(string LanguageId)
        {
            try
            {
                var filePath = Path.Combine(_langDir, $"{LanguageId}.json");

               System.Text.Json JsonSerializerOptions o = new JsonSerializerOptions
                {
                    ReadCommentHandling = JsonCommentHandling.Skip,
                    AllowTrailingCommas = true,
                };

                return JsonSerializer.Deserialize<Dictionary<string, string>>(File.ReadAllText(filePath), o);
            }
            catch
            {
